...
const CreateQuizForm = ({ classroomId, onQuizCreated }) => {
  ...
  const submitHandler = async (e) => {
    e.preventDefault();
    try {
      await API.post(`/api/classrooms/${classroomId}/quizzes`, { title, questions });
      toast.success('Quiz created!');
      onQuizCreated(); // Notify parent
    } catch (error) {
  ...
  return (
    <form onSubmit={submitHandler} className="space-y-4">
      ...
      {questions.map((q, qIndex) => (
        <div key={qIndex} className="p-4 border border-dashed border-stone-300 rounded-lg space-y-2">
          ...
          <input
            ...
            className="w-full px-2 py-1 border border-stone-200 rounded-md"
            required
          />
